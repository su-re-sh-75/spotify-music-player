...
<body>
    ...
    <header>
        <nav>
            ...
            <form action="search.php" class="search-bar" method="get">
                <input type="search" id="search" placeholder="Search your favourite song">
    ...
    <main>
        <section class="sidebar">
            ...
            <div class="library">
                ...
                <div class="playlist-sidebar" >
                    <a href="playlist_1.php?id=8" style="color:#1ed760;">Rewind Hits 2022</a>
                </div>
                <div class="playlist-sidebar">
        ...
        <section class="playlist-hero-sec">
            <?php 
                $playlist_1 = $conn->query('SELECT * FROM all_tracks WHERE id = 8');
                while($row = $playlist_1->fetch_assoc()):
            ?>
            <div class="playlist-info">
                <img src="<?= is_file(explode("?",$row['image_path'])[0]) ? $row['image_path'] : "images/leo-tamil-2023.webp" ?>" alt="playlist_1 thumbnail">
                <div class="playlist-info-text">
                    <p class="playlist-name"><?= $row['track_name'] ?></p>
                    <p class="playlist-artist"><?= $row['track_artist'] ?></p>
            ...
            <table class="playlist">
                ...
                <?php 
                    $music = $conn->query('SELECT * FROM playlist_1');
                    while($row = $music->fetch_assoc()):
                ?>
                <tr class="playlist_song" data-id="<?= $row['id'] ?>">
                    <td><?= $row['id'] ?></td>
                    <td><?= $row['track_name'] ?></td>
                    <td><?= $row['track_album'] ?></td>
