    ...
    <main>
        <section class="sidebar">
            <div class="browse">
                ...
                <a href="album.php" style="color:#1ed760;">Album</a>
        ...
        <section class="album-hero-sec">
            <?php 
                // $sql = "SELECT * FROM all_tracks WHERE track_artist != 'Playlist' GROUP BY track_album ORDER BY track_album ASC, track_name ASC";
                $sql = "SELECT * FROM all_tracks WHERE track_artist != 'Playlist' ORDER BY track_album";
                $music = $conn->query($sql);
                $currentAlbum = "";

                while ($row = $music->fetch_assoc()) {
                    // Check if the album has changed
                    if ($currentAlbum !== $row['track_album']) {
                        if ($currentAlbum !== "") {
                            // Close the previous album section
                            echo '</div></div>';
                        }
                        
                        // Start a new album section
                        echo '<div class="album-section">';
                        echo '<h2>' . $row['track_album'] . '</h2><div class="songs-contianer">';
                        $currentAlbum = $row['track_album'];
                    }
                    
                    // Display song info for the current album
