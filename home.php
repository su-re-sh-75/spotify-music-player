...
<body>
    ...
    <main>
        ...
        <section class="hero-sec">
            <?php 
                $music = $conn->query('SELECT * FROM all_tracks WHERE id = 8 OR id = 7 ORDER BY id DESC');
                while($row = $music->fetch_assoc()):
            ?>
            <div class="big-song" data-id="<?= $row['id'] ?>">
                <a class="track_name" href="playlist_<?= 9 - $row['id'] ?>.php?id=<?= $row['id'] ?>">
                    <img src="<?= is_file(explode("?",$row['image_path'])[0]) ? $row['image_path'] : "images/leo-tamil-2023.webp" ?>" alt="song logo">
                    <br><?= $row['track_name'] ?>
                </a>
            </div>
            <?php endwhile; ?>
            <div class="song-sections">
                <?php 
                    $music = $conn->query('SELECT * FROM all_tracks WHERE id != 8 AND id != 7 LIMIT 12');
                    while($row = $music->fetch_assoc()):
                ?>
