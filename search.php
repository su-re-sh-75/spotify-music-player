        ...
        <?php 
                ...
                if (isset($_GET['search'])) {
                    ...
                    $sql = "SELECT * FROM all_tracks
                            WHERE track_artist != 'Playlist' AND LOWER(track_name) LIKE LOWER('%$searchTerm%')";
                    // $sql = (SELECT * FROM all_tracks
                    //     WHERE track_artist != 'Playlist' AND LOWER(track_name) LIKE LOWER('%$searchTerm%')
                    // )
                    // UNION
                    ...
                    //     WHERE LOWER(track_name) LIKE LOWER('%$searchTerm%')
                    // )
                    // UNION
                    // (
                    //     SELECT * FROM playlist_2
                    //     WHERE LOWER(track_name) LIKE LOWER('%$searchTerm%')
                    // )";
                    $music = $conn->query($sql);

                    while ($row = $music->fetch_assoc()) {
                        $searchResults .= '<div class="song" data-id='.$row['id'].'>';
                        $searchResults .= '<a class="track_name" href="#"><img src="' . (is_file(explode("?", $row['image_path'])[0]) ? $row['image_path'] : "images/leo-tamil-2023.webp") . '" alt="song logo"><br>' . $row['track_name'] . '</a>';
                        $searchResults .= '<p class="composer">' . $row['track_artist'] . '</p>';
                        $searchResults .= '</div>';
                    }

                    echo $searchResults; 
                } else {
                    echo "<p class='track_name'>No search results found</p>";
                }
            ?>
        </section>
    </main>
